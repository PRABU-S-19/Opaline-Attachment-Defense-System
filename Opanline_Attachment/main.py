...
#Sandbox Isolation
def move_to_sandbox(file_path):
    sandbox_folder = "sandbox"
    os.makedirs(sandbox_folder, exist_ok=True)

    sandbox_path = os.path.join(sandbox_folder, os.path.basename(file_path))
    shutil.copy(file_path, sandbox_path)

    return sandbox_path

#Static PNG Rendering 
def render_pdf_to_png(file_path):
    output_folder = "safe_preview"
...
#Safe Delivery
def opaline_attachment(file_path):
    print("\n[1] Extracting attachment content...")
    text = extract_text(file_path)

    print("[2] Classifying using DistilBERT...")
    label, confidence = classify_attachment(text)

    print(f"Prediction: {label}")
    print(f"Confidence: {confidence:.2f}")

    if label in ["Suspicious", "Malicious"]:
        print("[3] Moving file to sandbox...")
        sandbox_path = move_to_sandbox(file_path)

        print("[4] Rendering safe PNG preview...")
        ext = os.path.splitext(file_path)[1].lower()

        if ext == ".pdf":
            preview = render_pdf_to_png(sandbox_path)
        else:
            preview = render_text_to_png(text)

        print(f"[5] Safe preview generated at: {preview}")
        print("WARNING: Attachment flagged as potentially malicious.")
        print("User confirmation required before opening original file.")

    else:
        print("File is safe. Delivering normally.")
...
def extract_structural_features(text):
    """
    Extract suspicious structural indicators
    Returns numerical feature vector
    """
    suspicious_keywords = [
        "macro", "enable content", "powershell",
        "cmd.exe", "javascript", "eval(",
        "base64", "shellcode", "payload",
        "download", "execute", "system("
    ]

    keyword_count = sum([text.count(word) for word in suspicious_keywords])
    ...
    if text_length > 0:
        structural_vector = structural_vector / text_length

    return structural_vector


def distilbert_feature_encoding(raw_text):
    #Clean Text
    cleaned_text = clean_text(raw_text)

    #Structural Feature Extraction
    structural_features = extract_structural_features(cleaned_text)
    ...
    # Convert structural features to tensor
    structural_tensor = torch.tensor(structural_features, dtype=torch.float32)
    structural_tensor = structural_tensor.unsqueeze(0)

    #Feature Fusion
    # Concatenate semantic embedding with structural features
    combined_features = torch.cat((cls_embedding, structural_tensor), dim=1)

    return combined_features

labels = ["Benign", "Suspicious", "Malicious"]

def classify_attachment(text):
    inputs = tokenizer(
        text,
        return_tensors="pt",
        truncation=True,
